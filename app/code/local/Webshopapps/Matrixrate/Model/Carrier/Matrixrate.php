	   ...
	   foreach ($ratearray as $rate)
		{
		   if (!empty($rate) && $rate['price'] >= 0) {
                ...
                if(strtolower($this->getConfigData('shipingprice_type')) != '' && strtolower($this->getConfigData('shipingprice_type')) == 'p') {
                    $quote = Mage::getModel('checkout/session')->getQuote();
                    $quoteData = $quote->getSubtotal();
                    if($rate['price'] == '') {
                        $rate['price'] = 0;
                    }
                    $rate['price'] = ($quoteData * $rate['price']) / 100;
                }
